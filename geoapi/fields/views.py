...
class FieldViewSet(viewsets.ViewSet):
    ...
    # fields that are "nearby" given point within a given distance
    ...
    # fields inside the parallelogram
    ...
    # fields that intersect with geometry
    @action(detail=False, methods=['post'])
    def intersect(self, request):
        ...
        geometry = GEOSGeometry(str(geometry_data), srid=4326)
        crop = request.query_params.get('crop')
